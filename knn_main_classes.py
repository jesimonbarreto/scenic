...
def train(
  ...
  for step in steps:
    ...
    total_correct_per_class = {k: jnp.zeros(num_classes) for k in ks}
    total_samples_per_class = jnp.zeros(num_classes)
    total_samples = 0
    ...
    for i in range(config.steps_per_epoch_eval):
      ...
      for k in ks:
        ...
        predictions = jnp.argmax(probas_for_k[k], axis=1)
        correct_predictions = predictions == label_eval
        # Contar acertos por classe
        for cls in range(num_classes):
            mask = label_eval == cls
            total_correct_per_class[k] = total_correct_per_class[k].at[cls].add(jnp.sum(correct_predictions[mask]))
            total_samples_per_class = total_samples_per_class.at[cls].add(jnp.sum(mask))
    ...
    accuracy_per_class = {k: total_correct_per_class[k] / total_samples_per_class for k in ks}
    # Cálculo da acurácia total por K
    total_accuracies = {k: jnp.mean(accuracy_per_class[k]) for k in ks}
    # Resultado
    print(f'total samples used {total_samples}')
    print("Acurácia total para diferentes valores de K:")
    
    
    dir_results = config.train_dir +'/result_class/'  # Padrão: './results'
    os.makedirs(dir_results, exist_ok=True)
    # Salvar os valores de acurácia por classe em um arquivo .npz
    save_path = os.path.join(dir_results, f'accuracy_per_class_step_{step}.npz')
    jnp.savez(save_path, **accuracy_per_class)
