...
class ViTDINO(nn.Module):
  ...
  def __call__(self, x: jnp.ndarray, *, inputs_kv: Optional[jnp.ndarray] = None,
    ...
    x_class = nn.Dense(self.n_classes_cls)(x_cls)
...
class ViTDinoModel(base_model.BaseModel):
  ...
  def cross_entropy_loss_fn(self, y_true: jnp.ndarray, y_pred: jnp.ndarray) -> jnp.ndarray:
    """Cross-entropy loss usando logits."""
    one_hot_labels = jax.nn.one_hot(y_true, num_classes=y_pred.shape[-1])
    loss = optax.softmax_cross_entropy(logits=y_pred, labels=one_hot_labels)
